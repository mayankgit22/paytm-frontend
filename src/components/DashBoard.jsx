...
function DashBoard() {
    ...
    useEffect(() => {
        const fetchBalance = async () => {
            try {
                const response = await fetch('http://localhost:3000/api/v1/user/account/balance', {
  ...
  return (
    ...
    <div className="bg-white w-64 h-80 rounded-lg shadow-md p-2 flex flex-col items-center justify-between mx-auto mt-5 border border-gray-200">
      ...
      <div className="w-full flex flex-col  gap-2">
        ...
        <button  onClick={handleSend} className="bg-blue-500 text-white rounded-md p-2 w-1/2">Send</button>
      </div>
    </div>
   {showPopup && (
